...
fun validateReleaseRoutingConfiguration() {
    require(osrmBaseUrl.startsWith("https://")) {
        "PMP_OSRM_BASE_URL must use HTTPS for release builds."
    }
    require(osrmBaseUrl.trimEnd('/') != "https://router.project-osrm.org") {
        "The shared public OSRM endpoint is forbidden in release builds. Configure PMP_OSRM_BASE_URL with your production routing service."
    }
    require(routingUserAgent != "ProfessionalMapPro/1.0") {
        "PMP_ROUTING_USER_AGENT must identify the production app and support contact."
    }
}
...
tasks.configureEach {
    if (name.contains("release", ignoreCase = true)) {
        doFirst {
            validateReleaseRoutingConfiguration()
        }
    }
}
