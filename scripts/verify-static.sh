...
echo "[1/20] Checking Kotlin package/path consistency and raw delimiter balance"
python3 - <<'PY'
...
echo "[2/20] Checking XML well-formedness"
python3 - <<'PY'
from pathlib import Path
...
echo "[3/20] Checking version catalog duplicate keys"
python3 - <<'PY'
from pathlib import Path
import sys
...
echo "[4/20] Syntax-checking C++/JNI"
...
echo "[5/20] Checking launcher scripts"
if [[ ! -x "gradlew" ]]; then
...
if ! grep -Fq "EXPECTED_WRAPPER_JAR_SHA256=497c8c2a7e5031f6aa847f88104aa80a93532ec32ee17bdb8d1d2f67a194a9c7" gradlew.bat || ! grep -Fq "Get-FileHash -Algorithm SHA256" gradlew.bat; then
  echo "gradlew.bat must verify the committed Gradle wrapper JAR checksum on Windows." >&2
  exit 1
fi
if grep -Fq '"%COMSPEC%" /c exit 1' gradlew.bat; then
  echo "gradlew.bat failure branches must terminate the parent script with exit /b 1." >&2
  exit 1
fi

echo "[6/20] Checking build and security configuration guardrails"
python3 - <<'PYSEC'
from pathlib import Path
import re
import sys
...
if 'distributionSha256Sum=bafc141b619ad6350fd975fc903156dd5c151998cc8b058e8c1044ab5f7b031f' not in wrapper:
    errors.append('gradle-wrapper.properties must pin the official Gradle 9.5.1 binary distributionSha256Sum.')
...
for module in ['app', 'benchmark', 'core/geo', 'core/location', 'core/observability', 'core/offline', 'core/service', 'feature/map']:
...
if 'org.gradle.java.installations.auto-download=false' not in props:
    errors.append('gradle.properties must disable hidden JDK toolchain downloads; JDK 17/21 is an explicit prerequisite.')
if 'foojay-resolver-convention' in Path('settings.gradle.kts').read_text():
    errors.append('settings.gradle.kts must not introduce an extra Foojay plugin-resolution dependency for this Android-only build.')
if Path('gradle/gradle-daemon-jvm.properties').exists():
    errors.append('gradle-daemon-jvm.properties must not pin or download a daemon JDK; use JAVA_HOME or Android Studio JBR.')
...
if 'classpath("com.google.gms:google-services:4.5.0")' not in root_gradle:
    errors.append('Top-level build.gradle.kts must use the official Google Services 4.5.0 classpath when google-services.json exists.')
if 'if (rootProject.file("app/google-services.json").isFile)' not in root_gradle:
    errors.append('Firebase Gradle plugins must be resolved only when a real app/google-services.json is present.')
app_gradle=Path('app/build.gradle.kts').read_text()
map_gradle=Path('feature/map/build.gradle.kts').read_text()
for token in ('tasks.configureEach', 'doFirst', 'PMP_VERSION_CODE', 'PMP_VERSION_NAME', 'validateReleaseConfiguration'):
    if token not in app_gradle:
        errors.append(f'app/build.gradle.kts release gate must include {token}.')
if 'gradle.taskGraph.whenReady' in app_gradle or 'gradle.taskGraph.whenReady' in map_gradle:
    errors.append('Release gates must use task-level doFirst validation; TaskExecutionGraph receiver lambdas are incompatible with the targeted Gradle Kotlin DSL and configuration cache.')
for token in ('tasks.configureEach', 'doFirst', 'PMP_OSRM_BASE_URL', 'PMP_ROUTING_USER_AGENT', 'validateReleaseRoutingConfiguration'):
    if token not in map_gradle:
        errors.append(f'feature/map/build.gradle.kts release routing gate must include {token}.')
if 'router.project-osrm.org' not in map_gradle or 'forbidden in release builds' not in map_gradle:
    errors.append('feature/map release gate must reject the shared public OSRM endpoint.')
...
performance_workflow=Path('.github/workflows/performance.yml')
...
benchmark_gradle = Path('benchmark/build.gradle.kts').read_text()
if 'testOptions.managedDevices.devices' in benchmark_gradle or 'create<ManagedVirtualDevice>' in benchmark_gradle:
    errors.append('benchmark/build.gradle.kts must use the AGP 8.1+ managedDevices.localDevices DSL without a ManagedVirtualDevice generic import.')
for token in ('testOptions {', 'managedDevices {', 'localDevices {', 'create("pixel6Api33")', 'systemImageSource = "aosp-atd"'):
    if token not in benchmark_gradle:
        errors.append(f'benchmark/build.gradle.kts managed-device configuration must include {token}.')
if not performance_workflow.exists():
    errors.append('A scheduled/manual Macrobenchmark workflow is required.')
else:
    performance_text=performance_workflow.read_text()
    for token in (
        ':benchmark:pixel6Api33BenchmarkAndroidTest',
        'workflow_dispatch:',
        'schedule:',
        'PMP_OSRM_BASE_URL:',
        ...
        if token not in performance_text:
            errors.append(f'Performance CI must include {token}.')
...
echo "[7/20] Optional pure Kotlin compile checks"
...
echo "[8/20] Checking module dependency graph"
./scripts/verify-architecture.sh

echo "[9/20] Checking presentation and SOLID architecture guardrails"
...
echo "[10/20] Checking map bootstrap boundaries"
python3 - <<'PYBOOT'
from pathlib import Path
import sys
errors=[]
...
echo "[11/20] Checking route geometry boundaries"
python3 - <<'PYGEOM'
from pathlib import Path
import sys
errors=[]
...
echo "[12/20] Checking production source cleanup"
python3 - <<'PY'
from pathlib import Path
import re, sys
errors=[]
...
# Reject a common malformed wildcard form before AGP/profgen sees it.
BASELINE_PROFILE="app/src/main/baseline-prof.txt"
if [[ ! -s "$BASELINE_PROFILE" ]]; then
  echo "Missing or empty $BASELINE_PROFILE" >&2
  exit 1
fi
if grep -n '\*\*\*' "$BASELINE_PROFILE"; then
  echo "Malformed Baseline Profile wildcard detected; use (**) for parameters and ** for return types." >&2
  exit 1
fi
if grep -Ev '^(H|S|P|HS|HP|SP|HSP)?L[^[:space:]]+;($|->[^[:space:]]+\([^[:space:]]*\)[^[:space:]]+)$' "$BASELINE_PROFILE" | grep -q .; then
  echo "Baseline Profile contains a malformed rule." >&2
  exit 1
fi

echo "[13/20] Checking test-only fakes stay out of production"
python3 - <<'PY'
from pathlib import Path
import sys
forbidden = (
...
echo "[14/20] Checking production hardening guardrails"
python3 - <<'PY2'
from pathlib import Path
import sys
errors=[]
...
required_docs=['README.md','SETUP.md','LICENSE.md','ARCHITECTURE.md']
...
if 'ignoredFinishedWorkIds' in offline_manager:
    errors.append('AndroidWorkManagerOfflineDownloadManager must not retain an unbounded set of historical finished WorkManager IDs.')
if 'if (info.state.isFinished && knownWorkIds[clientId] == null) return@forEach' not in offline_manager:
    errors.append('AndroidWorkManagerOfflineDownloadManager must ignore historical finished WorkInfo rows that are not current work for this process.')
if 'removeKnownWorkId(clientId, info.id)' not in offline_manager:
    errors.append('AndroidWorkManagerOfflineDownloadManager must remove the current work ID after processing a terminal WorkInfo update.')
...
if 'while (true)' in offline_poller or 'delay(' in offline_poller:
    errors.append('OfflineDownloadProgressPoller must observe callback-driven StateFlow updates without timer polling.')
for token in ('mapNotNull', 'distinctUntilChanged', 'filterNotNull', 'first()'):
    if token not in offline_poller:
        errors.append(f'OfflineDownloadProgressPoller must include event-driven completion token: {token}.')
...
store=Path('core/service/src/main/java/com/msa/professionalmap/core/service/data/EncryptedNavigationSessionStore.kt')
if 'START_STICKY' not in service or 'engine.startOrRestore()' not in service:
    errors.append('ForegroundNavigationService must restore an explicitly persisted session after a process restart.')
if 'private fun hasLocationPermission()' not in service or 'LocationPermissionLevel.None' not in service or 'failBeforeForeground' not in service:
    errors.append('ForegroundNavigationService must reject missing location permission before promoting a location foreground service.')
start_index=service.find('startForegroundImmediately(NavigationRuntimeRegistry.state.value.session)')
restore_index=service.find('engine.startOrRestore()')
if start_index < 0 or restore_index < 0 or start_index > restore_index:
    errors.append('ForegroundNavigationService must enter foreground before decrypting/parsing the persisted navigation session.')
if 'ServiceCompat.stopForeground(this, ServiceCompat.STOP_FOREGROUND_REMOVE)' not in service:
    errors.append('ForegroundNavigationService must remove its public notification during teardown.')
if 'notificationManager.cancel(config.notificationId)' not in service:
    errors.append('ForegroundNavigationService must explicitly cancel the navigation notification during onDestroy teardown.')
engine=Path('core/service/src/main/java/com/msa/professionalmap/core/service/data/NavigationRuntimeEngine.kt').read_text()
if 'fun pause(): Boolean' not in engine or 'reroutePolicy.reset()' not in engine:
    errors.append('NavigationRuntimeEngine.pause must reset pending reroute policy before entering paused mode.')
if 'runtime.status == NavigationServiceStatus.Active' not in engine:
    errors.append('NavigationRuntimeEngine must only execute a delayed reroute while runtime status is Active.')
if 'tracking.location != null -> onLocation(tracking.location)' in engine:
    errors.append('NavigationRuntimeEngine must copy cross-module nullable public properties to a local val before null-checking; direct smart casts are not stable across module boundaries.')
if 'val currentLocation = tracking.location' not in engine or 'currentLocation != null -> onLocation(currentLocation)' not in engine:
    errors.append('NavigationRuntimeEngine location collection must use a stable local currentLocation value before calling onLocation.')
if not store.exists() or 'AndroidKeyStore' not in store.read_text() or 'noBackupFilesDir' not in store.read_text():
    errors.append('Navigation session restore must use encrypted Android Keystore-backed no-backup storage.')
elif 'synchronized(StoreLock)' not in store.read_text():
    errors.append('EncryptedNavigationSessionStore must serialize AtomicFile and Android Keystore access across controller/service instances.')
offline_cipher=Path('core/offline/src/main/java/com/msa/professionalmap/core/offline/data/OfflineWorkPayloadCipher.kt').read_text()
if 'synchronized(KeyCreationLock)' not in offline_cipher:
    errors.append('OfflineWorkPayloadCipher must serialize Android Keystore key creation.')
if 'ContextCompat.startForegroundService' not in Path('core/service/src/main/java/com/msa/professionalmap/core/service/data/AndroidNavigationServiceController.kt').read_text():
    errors.append('Navigation start/restore must use ContextCompat.startForegroundService from the visible app edge.')
...
geojson_sources=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapGeoJsonSources.kt').read_text()
marker_layers=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapMarkerLayers.kt').read_text()
selected_poi_layers=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapSelectedPoiLayers.kt').read_text()
route_layers=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapRouteLayers.kt').read_text()
if 'withCluster(true)' not in geojson_sources or 'withClusterMinPoints(2)' not in geojson_sources:
    errors.append('Professional POI rendering must keep tuned GeoJSON clustering enabled.')
if 'PoiSelectedCoreLayer' not in selected_poi_layers or 'PoiSelectedSymbolLayer' not in selected_poi_layers:
    errors.append('Selected POIs must keep a high-contrast core and symbol above the selection halo.')
if 'RouteHitLayer' not in route_layers or 'width = 24f' not in route_layers:
    errors.append('Route alternatives must preserve a wide transparent hit target for reliable touch selection.')
if 'MapInteractionResolver' not in Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapLibreView.kt').read_text():
    errors.append('MapLibreView must preserve ordered POI/route hit testing.')
if 'RouteSnapOptions' not in Path('core/model/src/main/java/com/msa/professionalmap/core/model/RoutingModels.kt').read_text():
    errors.append('Routing must preserve provider-neutral road snapping hints.')
if 'NavigationReroutePolicy' not in engine:
    errors.append('Navigation runtime must use anti-thrashing reroute policy.')
if errors:
    print('\n'.join(errors))
    sys.exit(1)
...
echo "[15/20] Checking cleaned delivery scope"
python3 - <<'PY3'
from pathlib import Path
import sys
errors=[]
...
scan_roots=[Path('app'), Path('core'), Path('feature'), Path('gradle'), Path('.github'), Path('scripts')]
scan_files=[Path('settings.gradle.kts'), Path('build.gradle.kts'), Path('gradle.properties')]
for root in scan_roots:
    if root.exists():
        scan_files.extend(
            path for path in root.rglob('*')
            if path.is_file() and path.suffix in {'.kt', '.xml', '.md', '.kts', '.toml'}
        )
...
echo "[16/20] Checking source-size guardrails"
check_line_limit() {
  local file="$1"
  local limit="$2"
  if [[ ! -f "$file" ]]; then
...
echo "[17/20] Checking UI import hygiene"
for file in \
...
command_primitives="feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapCommandPrimitives.kt"
if grep -F "content: @Composable Column.() -> Unit" "$command_primitives" >/dev/null 2>&1; then
  echo "$command_primitives: Column is a composable function, not a receiver type; use ColumnScope." >&2
  exit 1
fi
for required in \
  "import androidx.compose.foundation.layout.ColumnScope" \
  "content: @Composable ColumnScope.() -> Unit"; do
  if ! grep -F "$required" "$command_primitives" >/dev/null 2>&1; then
    echo "$command_primitives: missing Compose scope declaration: $required" >&2
    exit 1
  fi
done

map_screen_content="feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapScreenContent.kt"
if grep -F "import androidx.compose.foundation.layout.matchParentSize" "$map_screen_content" >/dev/null 2>&1; then
  echo "$map_screen_content: matchParentSize is provided by BoxScope and must not be imported as a top-level extension." >&2
  exit 1
fi

navigation_panels="feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapNavigationGuidancePanels.kt"
if ! grep -F "internal fun ProgressState.navigationLabel" "$navigation_panels" >/dev/null 2>&1; then
  echo "$navigation_panels: navigationLabel is shared by multiple UI files and must have internal visibility." >&2
  exit 1
fi
...
required_files=[
    ...
    Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapVisualPrimitives.kt'),
    Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapSceneFactory.kt'),
    Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapNavigationCockpitOverlay.kt'),
    Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapFloatingFollowControl.kt'),
    Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapSelectedPoiLayers.kt'),
]
...
for token in ('BoxWithConstraints', 'MapHudOverlay', 'MapFloatingFollowControl', 'AnimatedContent', 'navigationBarsPadding', 'MapUiLayout.CompactPanelMaxWidth', 'MapUiLayout.ExpandedPanelMinWidth', 'MapUiLayout.ExpandedBreakpoint', 'Brush.verticalGradient', 'matchParentSize'):
...
hud_files = [
    'feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapHudOverlay.kt',
    'feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapCompactHudOverlay.kt',
    'feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapExpandedHudOverlay.kt',
    'feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapNavigationCockpitOverlay.kt',
]
hud = '\n'.join(Path(path).read_text() for path in hud_files)
for token in ('onRequestLocationPermission', 'onStartLocation', 'onCalculateRoute', 'onStartNavigation', 'onDownloadOfflineRoute', 'MapQuickActionBar', 'MapPrimaryAction', 'MapSecondaryAction', 'MapRouteFocusCard', 'MapSystemStatusStrip'):
    if token not in hud:
        errors.append(f'Map HUD overlay family must keep executive quick action token: {token}')
route_focus=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapRouteFocusCard.kt').read_text()
for token in ('MapRouteFocusCard', 'LinearProgressIndicator', 'selectedRouteSummary', 'progressOrNull', 'formatZero'):
    if token not in route_focus:
        errors.append(f'MapRouteFocusCard must keep premium route focus token: {token}')
cockpit=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapNavigationCockpitOverlay.kt').read_text()
for token in ('MapNavigationCockpitOverlay', 'animateFloatAsState', 'remainingDistanceMeters', 'nextInstruction', 'liveRegion'):
    if token not in cockpit:
        errors.append(f'Navigation cockpit must keep glanceable driving token: {token}')
floating_follow=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapFloatingFollowControl.kt').read_text()
for token in ('MapFloatingFollowControl', 'animateColorAsState', 'Role.Button', 'selected = following'):
    if token not in floating_follow:
        errors.append(f'Floating follow control must keep professional map-control token: {token}')
...
for token in ('MapGlassPanel', 'MapPanelHandle', 'MapControlPanelHeader', 'heightIn(max = maxPanelHeight)', 'LazyColumn', 'item(key =', 'MapPrimaryAction', 'MapSecondaryAction'):
...
if 'verticalScroll' in control:
    errors.append('MapControlPanel must use keyed LazyColumn items so off-screen control sections are not eagerly composed.')

scene_factory=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapSceneFactory.kt').read_text()
for token in ('rememberMapScene', 'remember(', 'activeRoutePoints', 'maneuverPoints', 'selectedPoi'):
    if token not in scene_factory:
        errors.append(f'MapSceneFactory must keep memoized map-scene token: {token}')
map_view=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapLibreView.kt').read_text()
for token in ('rememberUpdatedState', 'snapshotFlow', 'distinctUntilChanged', 'renderer.render'):
    if token not in map_view:
        errors.append(f'MapLibreView must keep stable render-collection token: {token}')
renderer=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapSceneRenderer.kt').read_text()
for token in ('preparedStyle', 'if (preparedStyle !== style)', 'routeLayers.ensure', 'markerLayers.ensure'):
    if token not in renderer:
        errors.append(f'MapSceneRenderer must keep one-time style preparation token: {token}')
geo_sources=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapGeoJsonSources.kt').read_text()
if 'withSynchronousUpdate' in geo_sources:
    errors.append('Dynamic MapLibre GeoJSON sources must stay asynchronous; synchronous updates can block the UI thread.')
camera=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapCameraController.kt').read_text()
for token in ('MinimumCameraIntervalMillis', 'previousTimestamp', 'for (index in 0 until points.lastIndex)'):
    if token not in camera:
        errors.append(f'MapCameraController must keep camera-allocation/throttle token: {token}')
marker_layers=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapMarkerLayers.kt').read_text()
selected_poi_layers=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapSelectedPoiLayers.kt').read_text()
if 'setMinZoom(12.25f)' not in marker_layers:
    errors.append('POI labels must retain a minimum zoom to control clutter and glyph-placement cost.')
if 'PoiSelectedLabelLayer' not in selected_poi_layers or 'textAllowOverlap(true)' not in selected_poi_layers:
    errors.append('Selected POIs must keep an always-readable high-contrast label layer.')
baseline=Path('app/src/main/baseline-prof.txt').read_text()
for token in ('ReadyMapContent', 'rememberMapScene', 'MapSceneRenderer;->render', 'MapGeoJsonSources;->update', 'MapCameraController;->update', 'MapNavigationCockpitOverlay', 'MapFloatingFollowControl', 'MapSelectedPoiLayers;->ensure'):
    if token not in baseline:
        errors.append(f'Baseline profile must include visual-performance hot path: {token}')
...
catalog_text=Path('gradle/libs.versions.toml').read_text()
map_gradle=Path('feature/map/build.gradle.kts').read_text()
if 'androidx-compose-animation' not in catalog_text or 'libs.androidx.compose.animation' not in map_gradle:
    errors.append('The map feature must declare Compose Animation explicitly for polished state transitions.')
scene_model=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapScene.kt').read_text()
actions_model=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapActionHandlers.kt').read_text()
if '@Immutable' not in scene_model or '@Immutable' not in actions_model:
    errors.append('Map render and action models must retain Compose stability annotations.')
if errors:
    print('\n'.join(errors))
    sys.exit(1)
...
expected_docs=(README.md SETUP.md LICENSE.md ARCHITECTURE.md)
actual_docs=$(find . -maxdepth 1 -type f -name "*.md" -printf "%f\n" | sort | tr "\n" " ")
expected_docs_sorted=$(printf "%s\n" "${expected_docs[@]}" | sort | tr "\n" " ")
if [[ "$actual_docs" != "$expected_docs_sorted" ]]; then
  echo "expected exactly four root Markdown files: ${expected_docs_sorted}; found: ${actual_docs}" >&2
  exit 1
fi
for doc in "${expected_docs[@]}"; do
  for forbidden in "TODO implement later" "// ... existing code" "// rest of code"; do
    if grep -F "$forbidden" "$doc" >/dev/null 2>&1; then
      echo "$doc: documentation must not contain placeholder token $forbidden" >&2
      exit 1
    fi
  done
done
for token in "Professional Map Pro" "Android" "حریم خصوصی" "AGPL-3.0"; do
  if ! grep -F "$token" README.md >/dev/null 2>&1; then
    echo "README.md: missing required documentation token: $token" >&2
    exit 1
  fi
done
for token in "راه‌اندازی" "Google Services plugin" "PMP_RELEASE_STORE_FILE" "چک‌لیست قبل از release"; do
  if ! grep -F "$token" SETUP.md >/dev/null 2>&1; then
    echo "SETUP.md: missing required documentation token: $token" >&2
    exit 1
  fi
done
for token in "GNU AFFERO GENERAL PUBLIC LICENSE" "Version 3, 19 November 2007" "Remote Network Interaction"; do
  if ! grep -F "$token" LICENSE.md >/dev/null 2>&1; then
    echo "LICENSE.md: missing required AGPL token: $token" >&2
    exit 1
  fi
done
for token in "core:service" "NavigationRuntimeEngine" "PrivacySanitizer" "StateFlow"; do
  if ! grep -F "$token" ARCHITECTURE.md >/dev/null 2>&1; then
    echo "ARCHITECTURE.md: missing required architecture token: $token" >&2
    exit 1
  fi
done
echo "Final four-file documentation policy checks passed."


# Regression guardrails for Kotlin 2.x / Compose DSL compile failures previously found in feature:map.
python3 - <<'PYMAPCOMPILE'
from pathlib import Path
import re
import sys

errors=[]
formatter=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/presentation/MapUiMessageServiceFormatter.kt').read_text()
if formatter.count('is MapUiMessage.PoiSelected ->') < 2:
    errors.append('MapUiMessageServiceFormatter must handle PoiSelected in both English and Persian exhaustive when expressions.')
geo=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapGeoJsonSources.kt').read_text()
if 'map(GeoPoint::toPoint)' in geo or 'map(GeoPoint::toMapLibrePoint)' in geo:
    errors.append('Do not use callable references to GeoPoint conversion extensions; Kotlin rejects member/extension references in this context. Use an explicit lambda.')
if '.toPoint()' in geo:
    errors.append('MapGeoJsonSources must use the explicit toMapLibrePoint conversion; unresolved or ambiguous toPoint calls are forbidden.')
if re.search(r'location\?\.bearingDegrees\s*!=\s*null[\s\S]{0,200}location\.bearingDegrees', geo):
    errors.append('Cache bearingDegrees in a local val before null-checking; cross-module public properties cannot be smart-cast.')
scene=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapScene.kt').read_text()
factory=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapSceneFactory.kt').read_text()
if 'val selectedRoute: RouteAlternative?' not in scene or 'selectedRoute = selectedRoute' not in factory:
    errors.append('MapScene must carry the memoized selected route so camera updates do not scan alternatives on every GPS frame.')
screen=Path('feature/map/src/main/java/com/msa/professionalmap/feature/map/ui/MapScreenContent.kt').read_text()
if re.search(r'\.padding\(\s*horizontal\s*=\s*[^,]+,\s*bottom\s*=', screen):
    errors.append('Compose padding has no horizontal+bottom overload; use start/end/bottom or nested padding calls.')
if errors:
    print('\n'.join(f'ERROR: {e}' for e in errors), file=sys.stderr)
    raise SystemExit(1)
print('Feature-map Kotlin compile regression guardrails passed.')
PYMAPCOMPILE
...
TEST_FILE_COUNT=$(find core feature app -type f -path "*/src/test/*" -name "*.kt" | wc -l | tr -d " ")
if [[ "$TEST_FILE_COUNT" -lt 35 ]]; then
  echo "expected at least 35 Kotlin test files, found $TEST_FILE_COUNT" >&2
  exit 1
fi
for task in ":core:offline:testDebugUnitTest" ":feature:map:testDebugUnitTest" ":app:lintDebug"; do
